...
import mn.icode.service.DepartmentsService;
...
public class DepartmentsRestController {
	...
	@PostMapping
	public ResponseEntity<DepartmentResponse> create(@Valid @RequestBody DepartmentsRequest request) {
		DepartmentResponse createdDepartment = departmentsService.create(request);

		return ResponseEntity.status(HttpStatus.CREATED).body(createdDepartment);
	}

	@PutMapping("/{id}")
	public DepartmentResponse update(@PathVariable Long id, @Valid @RequestBody DepartmentsRequest request) {
